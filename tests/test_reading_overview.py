...
def test_get_reading_overview_builds_activity_only(monkeypatch):
    ...
    # HF daily / conference collections were removed in the lite trim.
    assert "hf_daily" not in overview
    assert "collections" not in overview
    assert len(cursor.calls) == 1  # only the activity query runs
...
def test_list_marked_papers_returns_first_viewed_at(monkeypatch):
    ...
    # note rides along on the same SELECT; fake rows without the key degrade
    # to None instead of raising.
    assert "pm.note" in cursor.calls[0][0]
    assert items[0]["mark"]["note"] is None
    # arXiv publish date rides along too (subquery on arxiv_papers).
    assert "published_at" in cursor.calls[0][0]
    assert items[0]["paper"]["published_at"] is None
