  ...
  return (
    <>
      ...
      <section
        className={
          isExpanded
            ? 'fixed inset-4 z-50 flex h-auto w-auto max-h-[calc(100dvh-2rem)] flex-col overflow-hidden rounded-[28px] bg-white shadow-2xl ring-1 ring-black/10 sm:inset-6 lg:inset-10'
            : 'flex h-[calc(100dvh-12rem)] min-h-[min(22rem,calc(100dvh-12rem))] flex-col overflow-hidden rounded-[28px] bg-white shadow-sm ring-1 ring-black/5'
        }
      >
        ...
        <div className="flex min-h-12 items-center justify-between gap-3 [@media(max-height:720px)]:min-h-10">
          ...
          <div className="flex shrink-0 items-center gap-1.5">
            <Button
              variant="ghost"
              size="icon"
              className="h-9 w-9 rounded-full text-[#1f2a3a] hover:bg-[#f1f5f9] hover:text-[#0f172a]"
              onClick={() => setIsExpanded((v) => !v)}
              title={isExpanded ? '收起' : '放大'}
              aria-label={isExpanded ? '收起对话框' : '放大对话框'}
            >
