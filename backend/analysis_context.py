...
def build_multi_paper_context(papers: list[dict[str, Any]]) -> str:
    """Build a combined chat context for multiple papers.

    Uses each paper's metadata + AI analysis (no full text) to keep the total
    size within model limits when several papers are involved.
    """
    parts: list[str] = []
    for index, paper in enumerate(papers, start=1):
        title = _value(paper.get("title"))
        block = [
            f"=== 论文 {index}：{title} ===",
            f"会议：{_value(paper.get('venue'))}",
            f"领域：{_value(paper.get('primary_area'))}",
            f"作者：{_join_values(paper.get('authors'))}",
            f"关键词：{_join_values(paper.get('keywords'))}",
            f"摘要：{_value(paper.get('abstract'))}",
        ]
        analysis = paper.get("llm_response")
        if analysis:
