...
def delete_paper_content_cache(paper_id: str) -> None:
    """Delete the on-disk paper content cache (Jina Reader text + meta) for a paper."""
...
def cache_paper_content(paper_id: str, pdf_url: str, content: str, source: str = "jina_reader") -> None:
    ...
    # Defensive: strip lone surrogates so size_bytes / file writes never crash.
...
def get_or_cache_paper_content(paper_id: str, pdf_url: str) -> str:
    ...
    # PDF/Jina extraction can yield lone UTF-16 surrogates that crash UTF-8
    # encoding (size_bytes, file writes, LLM payloads). Replace them.
    content = content.encode("utf-8", "replace").decode("utf-8")
