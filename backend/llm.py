...
class LLMToolCallDelta:
    """One streaming increment of a tool call (OpenAI delta.tool_calls item)."""
    index: int
    id: str
    name: str
    arguments: str
...
def iter_llm_stream_chunks(chunk):
    ...
    for tool_call_delta in _extract_tool_call_deltas(delta):
...
def _extract_tool_call_deltas(delta) -> list[LLMToolCallDelta]:
    """Normalize streaming delta.tool_calls entries across SDK shapes."""
    deltas = getattr(delta, "tool_calls", None)
    if not deltas:
        return []
    result: list[LLMToolCallDelta] = []
    for entry in deltas:
        if isinstance(entry, dict):
            ...
            call_id = getattr(entry, "id", None) or ""
            function = getattr(entry, "function", None)
            name = getattr(function, "name", None) or ""
            arguments = getattr(function, "arguments", None) or ""
        result.append(LLMToolCallDelta(
            index=int(index),
            id=str(call_id),
            name=str(name),
